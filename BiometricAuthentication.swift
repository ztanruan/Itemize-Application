                    ...
                    if success {
                        ...
                        case .faceID, .touchID:
                            ...
                            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                                guard !lastAccessedUserName.isEmpty else { return }
                                let passwordItem = KeychainPasswordItem(service: KeychainConfiguration.serviceName, account: lastAccessedUserName, accessGroup: KeychainConfiguration.accessGroup)
                                do {
                                    let storedPassword = try passwordItem.readPassword()
                                    LoginService.callLogInService(userName: lastAccessedUserName, password: storedPassword, successBlock: successBlock, failureBlock: failureBlock)
