...
public func makeCall(number:String)
{
    ...
    if UIApplication.shared.canOpenURL(url!) {
        ...
        //Temp Comment//appDelegate.window.rootViewController.view.makeToast("This device unable to make call")
    }
}
...
func GetFirebaseDeviceToken() -> String {
    ...
    if UserDefaults.appObjectForKey(AppMessage.firebase_token) != nil {
        if let dicTokenString = UserDefaults.appObjectForKey(AppMessage.firebase_token) as? String {
...
//MARK: - Get Device Token For Notification
func GetDeviceToken() -> String {
    
    var device_token = "IOS1234567890IOS"
    
    if UserDefaults.appObjectForKey(AppMessage.device_token) != nil {
        if let dicTokenString = UserDefaults.appObjectForKey(AppMessage.device_token) as? String {
            device_token = dicTokenString
        }
    }
...
func GetDeviceToken_data() -> Data {
    
    var device_tokenData = Data()
    
    if UserDefaults.appObjectForKey(AppMessage.device_tokenData) != nil {
        if let dicTokenData = UserDefaults.appObjectForKey(AppMessage.device_tokenData) as? Data {
            device_tokenData = dicTokenData
        }
    }
    
    return device_tokenData
}
